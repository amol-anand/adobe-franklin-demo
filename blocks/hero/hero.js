...
export default async function decorate(block) {
  ...
  text.innerHTML = `
    <div class="text cmp-text__regular">
      <div class="cmp-text">
        <h1>${cfg.title}</h1>
      </div>
      <div class="cmp-text__vertical">
        <div class="cmp-text__vertical--line"></div>
        <div class="cmp-text__vertical--text">SCROLL</div>
      </div>
    </div>
  `

  const textDiv = text.querySelector('.cmp-text__regular > .cmp-text');
  ...
  video.innerHTML = `
    <div class="col-image image-clip-left">
      <video autoPlay="" playsInline="" loop="" preload="auto" muted="" class="cmp-hero__variation-video gif">
        <source data-label="${cfg.name}" data-title="Merative_FINAL_R1_high.mp4"
                src="${cfg.url}" type="video/mp4">
    ...
    <div class="button cmp-button--play-primary cmp-button--launch-video">
      <button type="button" class="cmp-button" aria-label="Play Video" data-cmp-clickable="">
                                <span class="cmp-button__details">
                                <span class="cmp-button__text">Watch the video</span>
                                <span class="cmp-button__duration">${cfg.length}</span>
                                </span>
        <span class="cmp-button__icon cmp-button__icon--play" aria-hidden="true"></span>
      </button>
    </div>
